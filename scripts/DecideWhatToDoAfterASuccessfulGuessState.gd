...
func on_stop_your_turn_button_pressed():
	...
	if onHoldGuessOpponentCardState.cardPickedDuringPlayerTurn:
		context.update_local_and_remote_hand_with_added_card(onHoldGuessOpponentCardState.cardPickedDuringPlayerTurn, context.get_node("PickedCardOverviewPosition").position, context.get_node("OpponentPickedCardOverviewPosition").position)
